// src/pages/admin/AdminPackCreatePage/AdminPackCreatePage.jsx
...
import styles from "./AdminPackCreatePage.module.css";
...
function AdminPackCreatePage() {
  ...
  // Fetch all products for selection on mount
  useEffect(() => {
    async function loadProducts() {
      ...
      try {
        ...
        setError(err.message || "Failed to load products listing.");
      } finally {
  ...
  // Available products are those not yet selected
  ...
  const buildFormData = () => {
    ...
    // Map selected products to match DB schema array structure
    ...
    // Images attachments & custom ordering resolution
    const imageOrder = [];
  ...
  const handleSubmit = async (event) => {
    ...
    if (!form.name || !form.description || !form.packPrice || !form.profit) {
      setError("Please complete all required forms.");
      return;
    }

    if (selectedProducts.length === 0) {
      setError("Please add at least one product to the bundle pack.");
      return;
    }

    if (imageList.length === 0) {
      setError("Please upload at least one cover image for this pack.");
      return;
    }
    ...
    try {
      ...
      await packService.createPack(payload);
      navigate("/admin/packs");
    } catch (err) {
      setError(err.message || "Failed to create pack. Please try again.");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className={styles.page}>
      <h1 className={styles.title}>{t("admin.packForm.createTitle") || "Create New Pack"}</h1>
          ...
          <div className={styles.fullWidthField} style={{ gridColumn: "1 / -1", marginTop: "var(--space-2)" }}>
            ...
            <div className={styles.uploadBox}>
              ...
              <span className={styles.hint} style={{ marginLeft: "var(--space-3)" }}>
                Select files to add. Drag cards above to rearrange their display order.
              </span>
            </div>
          </div>
        </div>
...
export default AdminPackCreatePage;
